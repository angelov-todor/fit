  ...
  return (
    <>
      ...
      <div
        className={
          expanded
            ? 'fixed inset-4 top-16 z-50 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-2xl flex flex-col'
            : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden'
        }
        {...(expanded ? { role: 'dialog', 'aria-label': 'Expanded map' } : {})}
      >
        <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <div className="flex items-center gap-4">
            <span>Track points: <strong className="text-slate-700 dark:text-slate-200">{trackPoints.length.toLocaleString()}</strong></span>
